...
as
  ...
  /**
   * Hello Vancouver why is it always rainy??? Returns true/false if APEX developer is enable
...
as
  ...
  /**
   * -- TODO mdsouza: documentaiton and example
   * [Left shift](https://developer.mozilla.org/en/docs/Web/JavaScript/Reference/Operators/Bitwise_Operators#Left_shift)
   *
   * @example
   *
...
as
  ...
  is
    ...
    begin
      a := oos_util_bit.bitand(
        rol32(a + oos_util_bit.bitor(oos_util_bit.bitand(b, c), oos_util_bit.bitand(-b - 1, d)) + x + h, r) + e,
         gc_bmax32);
      c := rol32(c, 10);
    end gg;
...
as
  ...
  -- TODO mdsouza: move this to the package header
  /**
   * A PL/SQL implementation of the [Google Authenticator](https://github.com/google/google-authenticator/wiki)'s Time-based One-Time
  ...
  begin
    ...
    -- TODO mdsouza: comment what this is doing? Padding
    while l_npos < length(l_szbits) loop
      -- TODO mdsouza: why is this a select into?
      -- TODO mdsouza: could we do in one query?
      select
        ltrim(
          to_char(
            bin_to_num(
    ...
    l_nepoch := oos_util_date.timestamp2epoch(current_timestamp + p_offset);
